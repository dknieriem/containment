...
{	
		...
		public override void Reason (GameObject game, GameObject npc)
		{
				...
				if (npcZed.MoveTowardsSoundsICanHear ()) {
						if (Vector3.Distance (npcZed.InterestLocation, npc.transform.position) > Zed.ConfrontMaximumDistance) {
								npcZed.SetTransition (Transition.InvestigateTransition);
						} else {
								npcZed.SetTransition (Transition.ConfrontTransition);
						}
						return;
				...
				if (npcZed.MoveTowardsOtherZeds ()) {
						...
						npcZed.SetTransition (Transition.HerdingTransition);
						return;
				}
				...
				if (npcZed.CountdownToRandomChange <= 0) {
						...
						if (f <= 1) {
								...
								npcZed.SetTransition (Transition.IdleToRoamTransition);
						}
				}
		}
