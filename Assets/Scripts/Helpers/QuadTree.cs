...
using System.Collections.Generic;
	...
	/// </summary>
	public interface IVector2Storable
	{
		/// <summary>
	...
	internal class QuadTreeObject<T> where T : IVector2Storable //, IComparable<QuadTreeObject<T>>
{
		/// <summary>
	...
	public class QuadTree<T> : ICollection<T> where T : IVector2Storable //IQuadStorable
{
		...
		{
		quadTreeRoot = new QuadTreeNode<T>(new Rect(x, y, width, height));
		}
		...
		public List<T> GetObjects(Rect rect)
		{
	...
	/// </summary>
	/// <typeparam name="T">Any object implementing IQuadStorable.</typeparam>
	public class QuadTreeNode<T> where T : IVector2Storable //IQuadStorable
{
		#region Constants
		...
		private Rect rect; // The area this QuadTree represents
		...
		private QuadTreeNode(QuadTreeNode<T> parent, Rect rect)
			: this(rect)
		{
			this.parent = parent;
		}
		...
		private void Subdivide()
		{
			...
			childTL = new QuadTreeNode<T>(this, new Rect(rect.x, rect.yMax, size.x, size.y));
			childTR = new QuadTreeNode<T>(this, new Rect(mid.x, rect.yMax, size.x, size.y));
			childBL = new QuadTreeNode<T>(this, new Rect(rect.x, mid.y, size.x, size.y));
			childBR = new QuadTreeNode<T>(this, new Rect(mid.x, mid.y, size.x, size.y));
		...
		private QuadTreeNode<T> GetDestinationTree(QuadTreeObject<T> item)
		{
			...
			if (childTL.QuadRect.Contains(item.Data.position))
			{
				destTree = childTL;
			}
			else if (childTR.QuadRect.Contains(item.Data.position))
			{
				destTree = childTR;
			}
			else if (childBL.QuadRect.Contains(item.Data.position))
			{
				destTree = childBL;
			}
			else if (childBR.QuadRect.Contains(item.Data.position))
			{
				destTree = childBR;
			}
		...
		private void Relocate(QuadTreeObject<T> item)
		{
			...
			if (QuadRect.Contains(item.Data.position))
			{
		...
		internal void Insert(QuadTreeObject<T> item)
		{
			...
			if (!rect.Contains(item.Data.position))
			{
		...
		internal List<T> GetObjects(Rect searchRect)
		{
		...
		/// <param name="searchRect">The rectangle to find objects in.</param>
		/// <param name="results">A reference to a list that will be populated with the results.</param>
		internal void GetObjects(Rect searchRect, ref List<T> results)
		{
			// We can't do anything if the results list doesn't exist
			if (results != null)
			{
				if (searchRect.Overlaps(this.rect))
				{
					...
					if (objects != null)
					{
						for (int i = 0; i < objects.Count; i++)
						{
							if (searchRect.Contains(objects[i].Data.position))
							{
